...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Move(Vector2 axisDirection)
    {
        ...
        else if (isPlayerClimbing)
        {
            ...
            Vector3 checkerLeftPosition = transform.position + (transform.up * 1) + (-transform.right * 2.5f);
            Vector3 checkerRightPosition = transform.position + (transform.up * 1) + (transform.right * 1.5f);
